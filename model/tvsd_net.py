...
from util.metric.misc import AvgMeter, cal_precision_recall_mae, cal_fmeasure, cal_jaccard, cal_ber
...
class LightningNetwork(pl.LightningModule):
    def __init__(self, configs: dict):
        ...
        self.binary_xloss = binary_xloss
        ...
        self.weight_decay = float(configs["weight_decay"])
        self.warm_up_epochs = configs["warm_up_epochs"]
        self.max_epochs = configs["max_epochs"]
        self.test_adjacent_length = configs["test_adjacent_length"]

        self.save_hyperparameters()

        self.precision_record = [AvgMeter() for _ in range(256)]
        self.recall_record = [AvgMeter() for _ in range(256)]
        self.mae_record = AvgMeter()
        self.jaccard_record = AvgMeter()
        self.ber_record = AvgMeter()

    def forward(self, x: torch.Tensor):  # type: ignore
        print("Didn't implement this forward function.")

    def training_step(self, batch: dict, batch_idx: int):  # type: ignore
        exemplar, exemplar_gt = batch['exemplar'], batch['exemplar_gt']
        query, query_gt = batch['query'], batch['query_gt']
        other, other_gt = batch['other'], batch['other_gt']

        exemplar_pre, query_pre, other_pre, scene_logits = self.backbone(exemplar, query, other)

        # segmentation loss
        bce_loss1 = self.binary_xloss(exemplar_pre, exemplar_gt)
        bce_loss2 = self.binary_xloss(query_pre, query_gt)
        bce_loss3 = self.binary_xloss(other_pre, other_gt)

        loss_hinge1 = self.lovasz_hinge(exemplar_pre, exemplar_gt)
        loss_hinge2 = self.lovasz_hinge(query_pre, query_gt)
        loss_hinge3 = self.lovasz_hinge(other_pre, other_gt)
        ...
        loss_cls = self.ce_loss(scene_logits, scene_labels) * 10
        ...
        logged_images = {
            "exemplar/pred": ((torch.sigmoid((exemplar_pre))[-1]) > 0.5).to(torch.int8),
            "exemplar/image": (self.reverse_normalize(exemplar[-1]).cpu().numpy()*255).astype(np.uint8),
            "exemplar/gt": exemplar_gt[-1],
            "query/pred": ((torch.sigmoid((query_pre))[-1]) > 0.5).to(torch.int8),
            "query/image": (self.reverse_normalize(query[-1]).cpu().numpy()*255).astype(np.uint8),
            "query/gt": query_gt[-1],
            "other/pred": ((torch.sigmoid((other_pre))[-1]) > 0.5).to(torch.int8),
            "other/image": (self.reverse_normalize(other[-1]).cpu().numpy()*255).astype(np.uint8),
            "other/gt": other_gt[-1],
            }
    ...
    def test_step(self, batch: dict, batch_idx: int):  # type: ignore
        exemplar, exemplar_gt = batch['exemplar'], batch['exemplar_gt']
        # w, h = batch['exemplar_w'], batch['exemplar_h']

        adjacent_length = self.test_adjacent_length - 1
        pred_list = []
        for i in range(adjacent_length):
            query = batch['query_{}'.format(i)]
        
            exemplar_pre, _, _, _ = self.backbone(exemplar, query, query)
            res = (exemplar_pre.data > 0).to(torch.float32)
            pred_list.append(res)
            print(1, res.shape)

        for batch_idx in range(exemplar_pre.size(0)):
            single_pred_list = []
            for i in range(adjacent_length):
                # # Resize to original size, and then calculate the BER will take some time. However, there is no huge different in the evaluation results. 
                # single_pred = np.array(
                #     transforms.Resize((int(h[batch_idx]), int(w[batch_idx])))(
                #         transforms.ToPILImage()(
                #             pred_list[i][batch_idx].cpu()))).astype(np.float32)
                single_pred = (pred_list[i][batch_idx].cpu().numpy() * 255.).astype(np.float32)
                single_pred_list.append(single_pred)
            fuse = (sum(single_pred_list) / adjacent_length).astype(np.uint8)
            
            # # # Resize to original size, and then calculate the BER will take some time. However, there is no huge different in the evaluation results. 
            # gt = np.array(
            #         transforms.Resize((int(h[batch_idx]), int(w[batch_idx])))(
            #             transforms.ToPILImage()(
            #                 exemplar_gt[batch_idx].cpu()))).astype(np.uint8)
            gt = (exemplar_gt[batch_idx].cpu().numpy() * 255.).astype(np.uint8)
            
            precision, recall, mae = cal_precision_recall_mae(fuse, gt)
            jaccard = cal_jaccard(fuse, gt)
            ber, _, _ = cal_ber(fuse, gt)

            for pidx, pdata in enumerate(zip(precision, recall)):
                p, r = pdata
                self.precision_record[pidx].update(p)
                self.recall_record[pidx].update(r)

            self.mae_record.update(mae)
            self.jaccard_record.update(jaccard)
            self.ber_record.update(ber)

    def test_epoch_end(self, outs):
        # log epoch metric
        self.log('mae_epoch', self.mae_record.avg)
        fmeasure = cal_fmeasure([precord.avg for precord in self.precision_record],
                        [rrecord.avg for rrecord in self.recall_record])
        self.log('f-beta_epoch', fmeasure)
        self.log('jaccard_epoch', self.jaccard_record.avg)
        self.log('ber_epoch', self.ber_record.avg)

    def configure_optimizers(self) -> torch.optim.Adam:
        # optimizer 
        params = [
            {"params": self.backbone.encoder.backbone.parameters(), "lr": self.finetune_learning_rate},
            {"params": self.backbone.encoder.aspp.parameters(), "lr": self.scratch_learning_rate},
            {"params": self.backbone.encoder.final_pre.parameters(), "lr": self.scratch_learning_rate},
            {"params": self.backbone.co_attention.parameters(), "lr": self.scratch_learning_rate},
            {"params": self.backbone.project.parameters(), "lr": self.scratch_learning_rate},
            {"params": self.backbone.final_pre.parameters(), "lr": self.scratch_learning_rate}
        ]
        optimizer = torch.optim.Adam(params, betas=(0.9, 0.99), eps=6e-8, weight_decay=self.weight_decay)
        
        # lr_scheduler
        warm_up_with_cosine_lr = lambda epoch: epoch / self.warm_up_epochs if epoch <= self.warm_up_epochs else 0.5 * \
                             (math.cos((epoch-self.warm_up_epochs)/(self.max_epochs-self.warm_up_epochs)*math.pi)+1)
    ...
    def pred_to_binary_image(self, pred):
        res = (pred.data > 0).to(torch.float32)
        return np.array(res.cpu())
